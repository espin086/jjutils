...
def download_playlist(playlists, mydirectory):
    ...
    for videos in playlists:
        directory = '{0}%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s'.format(mydirectory)
        os.system('youtube-dl --format mp4 --ignore-errors --write-info-json --download-archive ../logs/archive.txt -o "{0}" {1}'.format(directory, videos))
...
def gcs_uploader():
    ...
    for folder in ['20th\ Century\ Fox\ Trailers', 'HOT\ New\ Trailers\ \&\ Movieclips\ Exclusives',
            'Sony\ Pictures\ -\ Official\ Trailers', 'TRAILERS', 'Universal\ Pictures\ -\ Trailers',
            'Uploads\ from\ Paramount\ Pictures', 'Uploads\ from\ Walt\ Disney\ Studios']:
        print("INFO: folder to move data from {0}".format(folder))
        # --update only copies latest files
        os.system("cp --update ~/cronjob/merlin_ops/video_embedding/videos/raw_videos/{0}/* ~/cronjob/merlin_ops/video_embedding/videos/raw_videos/raw_videos".format(folder))
    #copy all files in raw videos to raw_video in cloud storage
    os.system('gsutil -m cp -n ~/cronjob/merlin_ops/video_embedding/videos/raw_videos/raw_videos/* gs://merlin-video/input/raw_videos')
