...
script_path = os.path.abspath("awssecretsmanager.py")

from awssecretsmanager import get_secret
...
def build_subject(name, role, company):
    ...
    subject = "{0}, I love the {1} role at {2}. Let's set up a call?".format(
        name, role, company
    )
    return subject
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Emails a recruiter about a job role I want to interview about"
    )
    parser.add_argument(
        "name", metavar="name", type=str, help="enter name of recruiter"
    )
    parser.add_argument(
        "email", metavar="email", type=str, help="enter email of recruiter"
    )
    parser.add_argument(
        "role", metavar="role", type=str, help="role you are interested in"
    )
